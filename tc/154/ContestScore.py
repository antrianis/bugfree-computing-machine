...
def do_test(data, __expected):
    startTime = time.time()
    instance = ContestScore()
    exception = None
    try:
        __result = instance.sortResults(data);
    except:
        import traceback
...
def run_tests():
    sys.stdout.write("ContestScore (1000 Points)\n\n")
    ...
    with open("ContestScore.sample", "r") as f:
        while True:
            label = f.readline()
