...
def do_test(board, __expected):
    startTime = time.time()
    instance = ConnectingGameDiv2()
    exception = None
    try:
        __result = instance.getmin(board);
    except:
        import traceback
...
def run_tests():
    sys.stdout.write("ConnectingGameDiv2 (1000 Points)\n\n")
    ...
    with open("ConnectingGameDiv2.sample", "r") as f:
        while True:
            label = f.readline()
